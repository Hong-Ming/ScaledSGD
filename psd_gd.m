function [X,fhist,ghist] = psd_gd(spmat, r, epochs, learning_rate, momentum, X0, lossfun)
% PSD_GD   Learn low-rank posdef matrix from samples of matrix elements
% X = PSD_GD(SPMAT, R, K) performs K steps of gradient descent
...
% 
% X = PSD_GD(SPMAT, R, K, LR) performs the above using learning rate LR.
% 
% X = PSD_GD(SPMAT, R, K, LR, MO) performs the above using learning 
% rate LR and momentum MO.
% 
% X = PSD_GD(SPMAT, R, K, LR, MO, X0) additionally uses X0 as the initial
% point, instead of the default random point.
% 
% X = PSD_GD(SPMAT, R, K, LR, MO, X0, lossfun) specify the loss function.
...
% 
% [X, FHIST] = PSD_GD(...) also returns the history of sum of square
% residuals sum_{i,j} |MAT(i,j) - SPMAT(i,j)|^2
% 
% [X, FHIST, GHIST] = PSD_GD(...) also returns the history of gradient
% norms 
...
w2 = fprintf('* Solver: GD,  Loss Function: %s loss',lossfun);
