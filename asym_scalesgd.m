function [U,V,fhist,ghist,ehist] = asym_scalesgd(M, r, epochs, learning_rate, lossfun, momentum, minibatch, reg, U0, V0, M_true, metric)
% ASYM_SCALESGD   Learn low-rank asymmetric matrix from samples of matrix elements
% X = PSD_SCALESGD(M, R, K) performs K epochs of preconditioned 
% stochastic gradient descent to compute an N1 x R factor U and N2 x R factor
% V so that MAT = U*V' approximately satisfies MAT(i,j) = M(i,j) for all 
% nonzero elements i,j in M. 
% M should be large and sparse.
% 
% X = ASYM_SCALESGD(M, R, K, LR) performs the above using learning rate LR.
% 
% X = ASYM_SCALESGD(M, R, K, LR, lossfun) specify the loss function.
...
% 
% X = ASYM_SCALESGD(M, R, K, LR, lossfun, MO, MI, REG) performs the above using
% momentum MO (default M0=0), minibatch size MI (default MI=1) and regularization 
% parameter REG (default REG=0).
% 
% X = ASYM_SCALESGD(M, R, K, LR, lossfun, MO, MI, REG, U0, V0) additionally uses
% U0 and V0 as the initial point, instead of the default random point.
% 
% X = ASYM_SCALESGD(M, R, K, LR, lossfun, MO, MI, REG, U0, V0, M_true) additionally 
% uses M_true to evaluate generalization error. If M_true is not specify, 
% M will be used instead.
% 
% X = ASYM_SCALESGD(M, R, K, LR, lossfun, MO, MI, REG, U0, V0, M_true, metric)
% specify the evaluation metric
%   Available evaluation metric:
%       'RMSE'    - root mean squre error (default)
...
if nargout == 5
    [it,jt,kt,Yt,mt] = RetriveDataEVAL(M_true);
end
...
% PU  = inv(U'*U);       % initail preconditioner
% PV  = inv(V'*V);       % initail preconditioner
...
w2 = fprintf('* Solver: Asym_ScaleSGD,  Loss Function: %s loss',lossfun);
...
if nargout == 5
    ...
    WordCount = fprintf('Epoch: %4d, Loss: %8.4e, MSE: %6.4f, Grad: %8.4e',0, ini_fhist, ini_ehist, ini_ghist);
else
    ini_ehist = inf;
    fprintf(repmat('\b',1,WordCount));
...
%     if minibatch > 1  || reg ~= 0  % with minibatch or regularization
        for batch = 1:ceil(m/minibatch)  % loop over each batch
            ...
            [gradU, gradV] = ComputePreGrad(bi,bj,bk,bY,batch_size);
...
%     else % no minibatch (will work this out later)
%         for idx = 1:m
%             bi = i(idx); bj = j(idx); bk = k(idx); bY = Y(idx);
%             ui = U(bi,:)'; uj = V(bj,:)'; uk = V(bk,:)';
%             % Compute gradient
%             switch lossfun
%                 case 'square'
%                     grad = U(bi,:)*V(bj,:)' - bY;
...
%     end
    ...
    if nargout == 5
        ...
        fprintf(repmat('\b',1,WordCount));
        WordCount = fprintf('Epoch: %4d, Loss: %8.4e, MSE: %6.4f, Grad: %8.4e',epoch, fhist(epoch), ehist(epoch), ghist(epoch));
    else
        fprintf(repmat('\b',1,WordCount));
...
function [gradU, gradV] = ComputePreGrad(i,j,k,Y,m) 
UUinv = U/(U'*U); VVinv = V/(V'*V); E = zeros(m,1);
UVinv = U/(V'*V); VUinv = V/(U'*U);
switch lossfun
    case 'square'
        for fdx = 1:m
            ii = i(fdx); jj = j(fdx); Mij = Y(fdx);
        ...
        Eij = sparse(i,j,E,n,n1,m);
        gradU = (1/m)*(Eij*VUinv)+reg*UUinv;
        gradV = (1/m)*(Eij'*UVinv)+reg*VVinv;
    case '1bit'
        for fdx = 1:m
            ii = i(fdx); jj = j(fdx); Yij = Y(fdx);
            RL = U(ii,:)*V(jj,:)';
    ...
    case 'pair'
        ...
        Eij = sparse(i,j,E,n,n1,m); Eik = sparse(i,k,E,n,n1,m);
        gradU = (1/m)*(Eij*VUinv-Eik*VUinv)+reg*UUinv;
        gradV = (1/m)*(Eij'*UVinv-Eik'*UVinv)+reg*VVinv;
    case 'rank'
        for fdx = 1:m
            ii = i(fdx); jj = j(fdx); kk = k(fdx); Yijk = Y(fdx);
            RL = Yijk*U(ii,:)*(V(jj,:)-V(kk,:))';
...
function [i,j,k,Y,m] = RetriveDataEVAL(M)
switch metric
    case 'RMSE'
        [i,j,val] = find(M);
        k = ones(1,numel(i));
